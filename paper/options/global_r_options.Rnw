...
<<global_r_options, tidy=FALSE, highlight=FALSE, include=FALSE, cache=FALSE, eval=TRUE>>=  
  ## Load packages 
  ...
  osp_distmap <- function(species=NULL){
    ...
    library(plyr) 
    library(rgbif)  
    out <- llply(species, function(x) occurrencelist(x, coordinatestatus = T, maxresults = 100)) 
    custom <- function(){list(theme_osp_distmap(), labs(colour = "Species"))}
