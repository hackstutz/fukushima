...
RF: laut der Formel oben eigentlich nicht. U(x) ist immer gleich nur p ändert sich aber das steht ausserhalb von U(). wollen wir das vielleicht simpler aufziehen a la aus 2 wurden 3 events? Vorschlag:
...
We use data from the International Social Survey Programme (ISSP) 2010. The data stem from interviews conducted with individuals from more than 40 different countries (mostly OECD). For the 2010 wave, the survey was extended by questions concerned with environmental attitudes. The variable of interest for our analyses is a five-scale item for the question \textbf{And do you think that nuclear power stations are...} ranging from \textit{extremely dangerous for the environment} (1) to \textit{not dangerous at all for the environment (5). We reverse the scale and interpret high values to indicate people with a high risk perception. 

For most countries the ISSP data provide the exact day on which the interview took place. As the data contain interviews contain both, interviews before and after the Fukushima accident, we have the interesting possibility to identify causal effects. We will do this using two approaches: (1) A fixed effects estimator and (2) a Regression Discontinuity Design. The two methods rely on slightly different assumptions and identification strategies, using both therefore provides information about the robustness of our results.

Natural Experiment
If we assume that none of the interviewed persons did anticipate the Fukushima accident, we might be able to estimate an average treatment effect (ATE) of the accident on the personal risk perception. The concept of a natural experiment assumes individuals were randomly assigned to either control or treatment group. In our setup the assignment depends on the time of the interview and leads to some pitfalls that need to be addressed. First, as can be seen from FIGURE_TIMELINES, data is clustered by countries, so comparing individuals before and after Fukushima means comparing individuals over different countries. However countries might strongly differ with regard to their baseline risk perception which directly leads to a bias when estimating the "Fukushima effect". Second, persons who within the interview period were interviewed later than others might systematically differ from persons who were interviewed early. For example busy high income people might be difficult to interview and therefore concentrate in the late end of the period. We address these problems using different identification strategies.

[FIGURE TIMELINES ABOUT HERE]

FE

Since country means of risk perception strongly differ, we need to make individuals from different countries comparable. One strategy is to estimate a country fixed effects model that demeans all model variables by countries, therefore eliminating all unobserved heterogeneity that is time-constant for each country. Within this frameworks, individuals across countries are rather comparable, however our estimate might still be biased by variables that vary over time and countries (e.g. by countries different trends due to political agendas). In addition, using fixed effects leads to many countries not having any variation within the treatment variable $D$ (being interviewed before or after the accident). The effect is therefore identified by only those countries that conducted interviews both before and after. These are Denmark, Switzerland, Slovenia and Canada. 
...
As described in the theory section, risk perception and the potential change of risk perception depends on several dimensions (e.g. age, sex and trust). To sum up our theoretic model we can derive the following hypotheses:
...
FIGURE_RDD_SEX ABOUT HERE
FIGURE_RDD_OLDYOUNG ABOUT HERE
FIGURE_RDD_HIGHLOW_SES ABOUT HERE
FIGURE_RDD_HIGHLOW_TRUST ABOUT HERE
