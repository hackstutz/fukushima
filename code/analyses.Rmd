...
betas30 <- c(fit_rd$est[1], fit_rd_male$est[1],fit_rd_female$est[1],fit_rd_young$est[1],fit_rd_old$est[1],fit_rd_uneduc$est[1],fit_rd_educ$est[1])
betas60 <- c(fit_rd$est[3], fit_rd_male$est[3],fit_rd_female$est[3],fit_rd_young$est[3],fit_rd_old$est[3],fit_rd_uneduc$est[3],fit_rd_educ$est[3])
coefnames <- c("Overall", "Males","Females","Young","Old","Less educated","More educated")
se30 <-  c(fit_rd$se[1], fit_rd_male$se[1],fit_rd_female$se[1],fit_rd_young$se[1],fit_rd_old$se[1],fit_rd_uneduc$se[1],fit_rd_educ$se[1])
se60 <-  c(fit_rd$se[3], fit_rd_male$se[3],fit_rd_female$se[3],fit_rd_young$se[3],fit_rd_old$se[3],fit_rd_uneduc$se[3],fit_rd_educ$se[3])
...
summary(lm(as.numeric(nuclear)~after+sex+age+DEGREE+trust+pol_trust+country+DATEMO*DATEYR, data=issp))
# the overall effect is 0.15 on the 1-5 scale. 

fit <- lm(as.numeric(nuclear)~after*(sex+age+DEGREE+trust+pol_trust)+country+DATEMO*DATEYR, data=issp)
summary(fit)
...
fit_final<-lm(as.numeric(nuclear)~as.numeric(before):(as.numeric(sex)+age+DEGREE+trust+pol_trust)+as.numeric(after):(as.numeric(sex)+age+DEGREE+trust+pol_trust)+country+DATEMO*DATEYR+country:as.numeric(date), data=issp)
...
```{r airplane_plot}
coefnames <- c("Female","Age (10y)","Lowest formal qualification","Intermediate secondary qualif.","Higher secondary","University incomplete","University","General Trust","Political Trust")
